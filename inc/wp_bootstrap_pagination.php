...
function conbusi_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'conbusi' ),
        'next_string'     => esc_html__( '&raquo;', 'conbusi' ),
        'before_output'   => '<div class="conbusi-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'conbusi_pagination_defaults', $defaults )
    );
