...
function conbusi_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'conbusi' ) . '</span></li>';
...
// Blog categories
function conbusi_featured_post_cat(){
...
// social media
if ( ! function_exists( 'conbusi_social' ) ) {
	function conbusi_social( $args = array() ) {
		...
		$url = conbusi_opt( 'conbusi_social_url' );
...
// header cart count
function conbusi_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( CONBUSI_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'conbusi' ); ?>">
...
function conbusi_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'conbusi_contact7_form_content', 10, 2 );