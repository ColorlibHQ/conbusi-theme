...
if ( ! function_exists( 'conbusi_breadcrumbs' ) ) {
    function conbusi_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'conbusi' )
        );
        $args      = apply_filters( 'conbusi_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<li class="breadcrumb-item"><a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'conbusi' ).' </a></li>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<li class="breadcrumb-item active" aria-current="page">'.esc_html__('Search results for:','conbusi') . get_search_query() . '</li>';
        } elseif ( is_404() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . esc_html__( 'Error 404', 'conbusi' ) . '</li>';
        } elseif ( is_home() ) {
            $html .= '<li class="breadcrumb-item active" aria-current="page">' . get_the_title( get_option( 'page_for_posts' ) ) . '</li>';
        }
        $html .= '</ol>';
        $html .= '</nav>';
        $html = apply_filters( 'conbusi_breadcrumbs_filter', $html );
