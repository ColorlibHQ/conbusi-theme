...
add_action( 'wp_enqueue_scripts', 'conbusi_sl_enqueue_scripts' );
function conbusi_sl_enqueue_scripts() {

	wp_enqueue_script( 'conbusi-post-likes', CONBUSI_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'conbusi-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'conbusi' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_conbusi_process_simple_like', 'conbusi_process_simple_like' );
add_action( 'wp_ajax_conbusi_process_simple_like', 'conbusi_process_simple_like' );
function conbusi_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'conbusi' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'conbusi' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'conbusi' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'conbusi' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'conbusi' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'conbusi' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'conbusi_show_user_likes' );
add_action( 'edit_user_profile', 'conbusi_show_user_likes' );
function conbusi_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'conbusi' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'conbusi' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
